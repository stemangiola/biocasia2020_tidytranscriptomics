...
```{r out.width = "70%", warning=FALSE, message=FALASE}
# identify some genes to label
topgenes <- c("CACNB2", "ZBTB16", "PRSS35", "PDPN", "SPARCL1", "DUSP1")
...
volcanoplot <- 
  ...
  # Subset data
	...
	mutate(symbol = ifelse(symbol %in% topgenes, as.character(symbol), "")) %>%
...
A really nice feature of using tidyverse and ggplot2 is that we can make interactive plots quite easily using the plotly package. This can be very useful for exploring what genes or samples are in the plots. We can make interactive plots directly from our ggplot2 object (volcanoplot). Having `label` in the `aes` is useful to visualise the identifier of the data point (here the gene symbol) or other variables when we hover over the plot.
...
volcanoplot %>% ggplotly(tooltip = c("text"))
```
...
## Calculation the logarithm of the scaled counts
