...
Background information and setup instructions for this workshop can be found [here](https://stemangiola.github.io/biocasia2020_tidytranscriptomics/articles/background.html).
...
Poll: How many differentially expressed transcripts are there for FDR < 0.05 if we did not include cell line in the formula?
```
...
```{r}
data("pasilla", package = "biocasia2020tidytranscriptomics")
counts_tt <-
    pasilla %>%
    tidybulk() %>%
...
de_gate <- 
  ...
  gate(
    feature,
    edgerQLT_PValue, 
    deseq2_pvalue, gate_list = biocasia2020tidytranscriptomics::de_gate_gates
  )
...
## Supplementary
Some things we don't have time to cover in Part 2 of this workshop can be found in the [Supplementary material](https://stemangiola.github.io/biocasia2020_tidytranscriptomics/articles/supplementary.html).
...
[Background information] (https://stemangiola.github.io/biocasia2020_tidytranscriptomics/articles/background.html#differences-between-bulk-and-single-cell-rna-sequencing-1)
...
tidySCE provides a bridge between the SingleCellExperiment single-cell package [@sce] and the tidyverse [@wickham2019welcome]. It enables the display the
...
# Calculation the logerithm of the scaled counts

We can treat `pbmc_small_polished` as a *SingleCellExperiment* object. 
From [here](https://www.bioconductor.org/packages/release/bioc/vignettes/SingleCellExperiment/inst/doc/intro.html)
...
We proceed to plot. _Note: we don't need to create many of these temporary variables. Here are created for educational purposes only._
...
```
# Plotting heatmap with tidyverse
